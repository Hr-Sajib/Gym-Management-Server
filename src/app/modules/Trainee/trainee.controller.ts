...
const enrollTraineeInClass = catchAsync(async (req: Request, res: Response) => {
  ...
  if (loggedInUserRole !== 'ADMIN' && loggedInUserRole !== 'TRAINEE') {
    throw new AppError(httpStatus.UNAUTHORIZED, 'Only admins or trainees can enroll in a class!');
  }
  if (loggedInUserRole === 'TRAINEE' && traineeId !== req.user?.userId) {
    throw new AppError(httpStatus.UNAUTHORIZED, 'Trainee can only enroll themselves!');
  }

  const updatedClass = await traineeServices.enrollTraineeInClass(classId, traineeId);
  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
...
const unenrollTraineeFromClass = catchAsync(async (req: Request, res: Response) => {
  const { classId } = req.params;
  const { traineeId } = req.body;
  const loggedInUserRole = req.user?.role;
  ...
  if (loggedInUserRole !== 'ADMIN' && loggedInUserRole !== 'TRAINEE') {
    throw new AppError(httpStatus.UNAUTHORIZED, 'Only admins or trainees can unenroll from a class!');
  }
  if (loggedInUserRole === 'TRAINEE' && traineeId !== req.user?.userId) {
    throw new AppError(httpStatus.UNAUTHORIZED, 'Trainee can only unenroll themselves!');
  }

  const updatedClass = await traineeServices.unenrollTraineeFromClass(classId, traineeId);
  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
